...
(async () => {
    ...
    w_obj = await inquirer.prompt({name: 'tpl_path', message: '输入模板项目路径（绝对路径，暂不支持URL）',default:'/Users/apple/Documents/GitHub/poetic-hous/'}, '');
    if (!w_obj.tpl_path) {
    ...
    spinner.succeed( '开始复制模板文件到当前项目路径...')
    try{
         cpSync(new_item_config.tpl_path,new_item_path,{recursive:true,force:true,filter(source, destination) {

                let exList = [
                    '/Users/apple/Documents/GitHub/poetic-hous/.git.*?',
                    '/Users/apple/Documents/GitHub/poetic-hous/node_modules.*?',
                    '/Users/apple/Documents/GitHub/poetic-hous/android.*?',
                    '/Users/apple/Documents/GitHub/poetic-hous/ios.*?',
                ];
                for(let i of exList){
                  if(source.match(i) ) {
    ...
    try{
        ...
        spinner.succeed(chalk.yellow('2.iOS端使用Xcode打开xwork:space!!!!文件后，设置开发者签名与BundleID以及权限'))
        spinner.succeed(chalk.yellow('3.设置firebase与woo后端,提示：下载对应的service文件'))
        spinner.succeed(chalk.yellow('4.遇到问题? self'))
        process.exit();
    }catch (e){
